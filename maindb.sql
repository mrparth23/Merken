...
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        IF NEW.board_name <> OLD.board_name THEN
            ...
            VALUES (OLD.user_id, OLD.board_id, OLD.board_name, NEW.board_name, CURRENT_TIMESTAMP,
                    'UPDATE', 'Board');
        END IF;
        IF NEW.description <> OLD.description THEN
            INSERT INTO user_logs(user_id, board_id, old_data, new_data, log_date, type, content)
            VALUES (OLD.user_id, OLD.board_id, OLD.description, NEW.description, CURRENT_TIMESTAMP,
                    'UPDATE', 'Board');
        END IF;
        IF NEW.status <> OLD.status THEN
    ...
    IF (TG_OP = 'INSERT') THEN
        ...
        VALUES (NEW.user_id, NEW.board_id, NEW.board_name, NEW.description, CURRENT_TIMESTAMP,
                'INSERT', 'Board');
    END IF;
    RETURN NEW;
END;
...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        IF NEW.page_name <> OLD.page_name THEN
            INSERT INTO user_logs(user_id, board_id, page_id, old_name, new_name, log_date, type,
                                  content)
            VALUES (var_r.user_id, OLD.board_id, OLD.page_id, OLD.page_name, NEW.page_name,
                    CURRENT_TIMESTAMP, 'UPDATE', 'Pages');
        END IF;
        IF NEW.page_data <> OLD.page_data THEN
            INSERT INTO user_logs(user_id, board_id, page_id, old_data, new_data, log_date, type,
                                  content)
            VALUES (var_r.user_id, OLD.board_id, OLD.page_id, OLD.page_data, NEW.page_data,
                    CURRENT_TIMESTAMP, 'UPDATE', 'Pages');
        END IF;
        IF NEW.status <> OLD.status THEN
            INSERT INTO user_logs(user_id, board_id, page_id, log_date, type, content)
            VALUES (var_r.user_id, OLD.board_id, OLD.page_id, CURRENT_TIMESTAMP, 'REMOVED',
                    'Pages');
        END IF;
    END IF;
    IF (TG_OP = 'INSERT') THEN
        INSERT INTO user_logs(user_id, board_id, page_id, new_name, log_date, type, content)
        VALUES (var_r.user_id, NEW.board_id, NEW.page_id, NEW.page_name, CURRENT_TIMESTAMP,
                'INSERT', 'Pages');
    END IF;
    RETURN NEW;
END;
...
DECLARE
    var_r record;
BEGIN
    FOR var_r IN (SELECT log_date FROM user_logs WHERE user_logs.user_id = userid)
        LOOP
            month_year :=
                    CONCAT(TO_CHAR(var_r.log_date, 'Month'), ' ', TO_CHAR(var_r.log_date, 'YYYY'));
            RETURN NEXT;
        END LOOP;
END;
$BODY$
...
DECLARE
    ...
    b_name varchar;
BEGIN
    FOR var_r IN (SELECT * FROM user_logs WHERE user_logs.user_id = userid)
        LOOP
            log_content := var_r.content;
            IF var_r.type = 'UPDATE' THEN
                IF var_r.old_name <> var_r.new_name THEN
                    IF var_r.content = 'Pages' THEN
                        log_text := CONCAT('Page name changed from ', var_r.old_name, 'to ',
                                           var_r.new_name);
                    ELSE
                        log_text := CONCAT('Board name changed from ', var_r.old_name, 'to ',
                                           var_r.new_name);
                    END IF;
                ELSE
                    IF var_r.content = 'Pages' THEN
            ...
            log_time :=
                    CONCAT(TO_CHAR(var_r.log_date, 'HH12:MI'), ' ', TO_CHAR(var_r.log_date, 'AM'));
            month_year :=
                    CONCAT(TO_CHAR(var_r.log_date, 'Month'), ' ', TO_CHAR(var_r.log_date, 'YYYY'));
            RETURN NEXT;
        END LOOP;
...
CREATE
    OR REPLACE
    FUNCTION public.user_profile_updates(userid integer,
...
BEGIN
    ...
    IF passwd !=
       '89e16254ebad5cba72e0aebe1614c7f9b795a7505f2637206ce10a3449a2b8bb23875b089124a8269fc0896494bab580c48ec17ef0c9187c86f9c9f8e4f5c5b7' THEN
        UPDATE users
        SET pwd = passwd
        WHERE user_id = userid;
    END IF;
